...
#define PORTB_DIR CONCBIT(DIO_PIN7_DIR, DIO_PIN6_DIR, DIO_PIN5_DIR, DIO_PIN4_DIR, DIO_PIN3_DIR, DIO_PIN2_DIR, DIO_PIN1_DIR, DIO_PIN0_DIR)
...
void DIO_voidSetPIN (u8 local_pinNumber, u8 local_pinValue)
{
	...
	else if (local_pinNumber < 8)
	{
		if (local_pinValue == DIO_HIGH)
			SET_BIT (PORTB, local_pinNumber);
		else
			CLR_BIT (PORTB, local_pinNumber);
	}
...
u8 DIO_u8GetPin(u8 local_pinNumber)
{
	...
	{
			if ((GET_BIT(PINB, local_pinNumber))==1)
				ret=DIO_HIGH;
			else
				ret=DIO_LOW;
	}
